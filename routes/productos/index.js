...
/*router.get('/materials/p/:p/l/:l',(req,res)=>{
							...
							if(e.custom_fields.length>0){
								e.custom_fields.forEach(cf=>{
									if(String(cf.name).toUpperCase()=='MATERIALS'){ //si tiene el custom field materials
										...
										if(flagMaterial){
											filterResponseCount++
											if(filterResponseCount/l > p-1 && filterResponseCount/l <= p){
												(e.brand_id>0)?e.brand=brands.find(b=>b.id==e.brand_id):e.brand=null
												filterResponse.push(e)
											}		
										}
									}
								})
							}
							if(filterResponseCount == l*p){
	...
	if(query){
		...
		var url=constantes.url_v3+"catalog/products/?limit=250&page="+pageToSearch+"&is_visible=true&include=custom_fields"
	}
